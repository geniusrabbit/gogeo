// Command genregions builds packed ISO 3166-2 tables from Debian iso-codes JSON.
package main
...
func main() {
	src := flag.String("src", "data/iso_3166-2.json", "ISO 3166-2 JSON")
...
func loadISO(path string) ([]isoRegion, error) {
	...
	if len(file.Items) == 0 {
		return nil, fmt.Errorf("no ISO 3166-2 records in %s", path)
	}
...
func build(srcPath string, items []isoRegion, ccToID map[string]uint8) ([]byte, error) {
	...
	for _, item := range items {
		...
		byCC[cc] = append(byCC[cc], packedRegion{
			name:    item.Name,
			code:    item.Code,
			cc:      cc,
			suffix:  suf,
	...
	buf.WriteString("var regionCodes = [...]string{\n")
	for _, r := range packed {
		fmt.Fprintf(&buf, "\t%s,\n", strconv.Quote(r.code))
	}
	buf.WriteString("}\n\n")

	buf.WriteString("var regionTypes = [...]string{\n")
	...
	for _, r := range packed {
		var suf [3]byte
		copy(suf[:], r.suffix)
		fmt.Fprintf(&buf, "\t{ID: %d, Name: %s, parent: %d, country: %d, typeID: %d, sufN: %d, suffix: [3]byte{%s}},\n",
			r.id,
